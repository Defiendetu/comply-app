  ...
  try {
    ...
    if (d.certificadoContraparteBase64) {
      ...
      if (!apiKey) {
        ...
        errors.push('ANTHROPIC_API_KEY tiene formato invalido (debe empezar con sk-ant-)');
      } else {
        try {
          const pdfSize = d.certificadoContraparteBase64.length;
          errors.push(`DEBUG: PDF size=${pdfSize}, API key starts with ${apiKey.substring(0, 12)}...`);
